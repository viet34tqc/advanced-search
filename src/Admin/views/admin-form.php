...
<div class="wrap">
	...
	<?php
		...
		$plugin_options = get_option( 'nv-search' );

		// add the nonce, option_page, action and referer.
		settings_fields( 'nv-search-settings' );
		do_settings_sections( 'nv-search-settings' );
		...
		foreach ( $post_types  as $post_type ) {
			...
			<fieldset>
				...
				<label for="<?php echo esc_attr( 'nv-search' . '_' . $the_post_type ); ?>">
					<input type="checkbox" id="<?php echo esc_attr( 'nv-search' . '-' . $the_post_type ); ?>" name="<?php echo 'nv-search' . '[' . esc_attr( $the_post_type ) . ']'; ?>" value="<?php echo esc_attr( $the_post_type ); ?>" <?php checked( $option_checked, 1, true ); ?> />
